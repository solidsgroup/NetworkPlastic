...
%     Fstarp = reshape(Fstar(:,:,3),[3,3]);
    ...
    phi_h = phi_h1*abs(h(t))^n + phi_h2*abs(h(t))^m;
    dh12 = -1/phi1*(dhpq + phi0 + phi_h);
    dh21 = -1/phi1*(dhqp - phi0 - phi_h);
...
plot(time,V2,'LineWidth',1.3)
title('Full hardening potential')
xlabel('Time')
ylabel('Volume')
legend('V_1','V_2','Location','NorthWest')
grid on
hold off
figure(4)
