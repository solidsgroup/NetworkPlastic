...
namespace mygal
{
    ...
    public:
        ...
        std::vector<T> computeArea() const
        {
            auto a = std::vector<T>();
            for (const auto& face : mFaces)
            {
        ...
        std::vector<T> computeAngle() const
        {
            auto th = std::vector<T>();
            for (const auto& face : mFaces)
            {
                auto angle = static_cast<T>(0.0);
                auto halfEdge = face.outerComponent;
                // Compute area of the face
                do
                {
                    auto angle = halfEdge->origin->point.getAngle(halfEdge->destination->point);
                    th.push_back(angle);
                    halfEdge = halfEdge->next;
                } while (halfEdge != face.outerComponent);
            }
        ...
        std::vector<std::vector<mygal::Vector2<double>>>  ReturnV() const
        {
            std::vector<std::vector<mygal::Vector2<double>>> listVert;
            std::vector < mygal::Vector2<double>> temp;
            for (const auto& face : mFaces)
            {
                auto halfEdge = face.outerComponent;
                temp.clear();
                // get all vertices by polygon
                do
                {
                    temp.push_back( halfEdge->origin->point);
                    halfEdge = halfEdge->next;
                } while (halfEdge != face.outerComponent);
                listVert.push_back(temp);
            }
            return listVert;
        }
